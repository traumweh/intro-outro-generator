...
"""
Generate intros by rendering a webpage containing frame-logic for animation
using https://github.com/dtinth/html5-animation-video-renderer
"""
...
parser = argparse.ArgumentParser(
    description="C3VOC Intro-Outro-Generator - Variant to use with webpages",
    usage="./make-html5-animation-video-renderer.py yourproject/ https://url/to/schedule.xml",
...
parser.add_argument(
    ...
    type=str,
    help="""
    Path to your project folder with an intro/outro/pause/bgloop.html (and optionally js, css, ...)
    """,
)
parser.add_argument(
    "schedule",
...
parser.add_argument(
    ...
    Usage: ./make-html5-animation-video-renderer.py yourproject/ --debug
    """,
)
...
parser.add_argument(
    ...
    Usage: ./make-html5-animation-video-renderer.py yourproject/ --id 4711 0815 4223 1337
    """,
)
...
parser.add_argument(
    ...
    This argument must not be used together with --debug
    Usage: ./make-html5-animation-video-renderer.py yourproject/ --room "HfG_Studio" "ZKM_Vortragssaal"
    """,
)
...
parser.add_argument(
    ...
    This argument must not be used together with --debug
    Usage: ./make-html5-animation-video-rendere.py yourproject/ --day "1" "3"
    """,
)
...
parser.add_argument(
    ...
    default=False,
    help="""
    Render a pause loop from the pause.html file in the project folder.
    """,
)
...
parser.add_argument(
    ...
    default=False,
    help="""
    Render outro from the outro.html file in the project folder.
    """,
)
...
parser.add_argument(
    ...
    default=False,
    help="""
    Render background loop from the bgloop.html file in the project folder.
    """,
)
...
if args.debug:
    ...
    events = [
        {
            "id": 80,
            "title": "Für 'n Appel und 'n Ei: die Vereinnahmung des öffentlichen Raumes durch digitale Werbung",
            "subtitle": "",
            "persons": persons,
...
def enqueue_job(event):
    ...
    if (
        ...
        return
    render_type = event_id
    url_query = "?render"

    if event_id not in ["pause", "outro", "bgloop"]:
        render_type = "intro"
        title = quote_plus(event["title"])
        speaker = quote_plus(event["personnames"])
        url_query += f"&title={title}&speaker={speaker}"

    html_src = os.path.abspath(
        os.path.join(args.project, render_type + ".html")
    )
    intermediate_clip = os.path.join(tempdir.name, event_id + ".ts")
    project_url = f"file://{ html_src }{ url_query }"
...
if args.ids:
    if len(args.ids) == 1:
        print("enqueuing {} job into node".format(len(args.ids)))
    else:
        print("enqueuing {} jobs into node".format(len(args.ids)))
else:
    if len(events) == 1:
        print("enqueuing {} job into node".format(len(events)))
    else:
        print("enqueuing {} jobs into node".format(len(events)))
    ...
    if not args.nof:
        ...
        if platform.system() == "Windows":
            ...
            final_clip = os.path.join(
                os.path.dirname(args.project), event_id + ".avi"
            )
        else:
            intermediate_clip = os.path.join(tempdir.name, event_id + ".mkv")
            final_clip = os.path.join(
                os.path.dirname(args.project), event_id + ".mkv"
            )
